...
### Граница событий overlay и приложения

Shadow DOM изолирует стили, но сам по себе не останавливает DOM-события. Поэтому
Visual Intent помечает корневой host стабильным атрибутом
`data-visual-intent-root="v1"`, интерактивные области —
`data-visual-intent-surface`, а composer — отдельным
`data-visual-intent-composer="v1"`.

В bubble-фазе ShadowRoot останавливает дальнейшее всплытие событий мыши,
указателя, контекстного меню и фокуса из интерактивных поверхностей. Для
composer дополнительно изолируются keyboard, input, composition и clipboard
события. Граница вызывает только `stopPropagation()`: она не использует
`preventDefault()` и не мешает нативному вводу текста, выделению, вставке или
клику самой панели. Глобальные горячие клавиши Visual Intent продолжают работать
вне редактируемых полей.

Capture-listener приложения срабатывает раньше, чем событие достигнет
ShadowRoot, поэтому overlay физически не может остановить его постфактум. Если
потребитель уже использует внутренний пакет `@visual-intent/web-overlay`, он
может импортировать готовый `isVisualIntentOverlayEvent`. Проекту без этой
зависимости достаточно нулевого по зависимостям guard по полному
`event.composedPath()`:

```ts
function isVisualIntentOverlayEvent(event: Event): boolean {
  return event
    .composedPath()
    .some(
      (node) =>
        node instanceof Element &&
        node.getAttribute("data-visual-intent-root") === "v1",
    );
}
...
document.addEventListener(
  ...
  (event) => {
    ...
    // Логика outside-dismiss хост-приложения.
  },
  true,
);
```

Тот же guard применяется в capture-phase глобальных keyboard listeners. Нельзя
определять принадлежность через `event.target.closest(...)`: за границей Shadow
DOM target ретаргетируется, тогда как `composedPath()` сохраняет корневой marker.

### Постоянный локальный процесс на macOS

Foreground CLI остаётся простейшим runtime, но не подходит для сессии, которая
должна переживать закрытие терминала. Команда `service install` создаёт один
пользовательский LaunchAgent на точный канонический корень репозитория. Label
состоит из безопасного slug проекта и hash пути, поэтому несколько проектов не
конфликтуют.

LaunchAgent запускает абсолютные Node.js и CLI напрямую, без shell-команды,
задаёт рабочую директорию, `HOME`, безопасный `PATH` и необязательный
`CODEX_HOME`. Target dev server в lifecycle не входит. Перед установкой и
запуском проверяются project lease, совместимое живое legacy-подключение,
резервации репозитория и порта. Успешный старт требует не только PID, но и
health-identity с ожидаемыми daemon instance, session, repository root и proxy
origin. PID LaunchAgent обязан совпасть с владельцем daemon lease, а ID instance
в lease — с health-ответом: старый процесс на том же порту не может подтвердить
готовность нового. При ошибке bootstrap или последующей readiness-проверки
загруженный job останавливается; если обратный `bootout` не удался, plist
и `service.json` сохраняются как точки управления потенциально живым
LaunchAgent. Короткая `.visual-intent/service-operation-lease.json`
сериализует все изменяющие lifecycle-команды (`install`, `start`, `stop`,
`restart`, `uninstall`), а конфигурация публикуется атомарно до plist: после
аварийного обрыва `status`, `stop` и `uninstall` всё равно могут найти точный
label. При штатном rollback прежний конфиг восстанавливается. Install,
остановка, restart и uninstall блокируются во время активного Apply, если
пользователь явно не передал аварийный `--force`.

Конфигурация находится в `.visual-intent/service.json` и имеет права `0600`.
Uninstall удаляет только LaunchAgent: проектные данные, логи и connection-файл
сохраняются, потому что последний может принадлежать другому foreground daemon.
Устаревшее подключение обнаруживается по instance/session/root через `doctor`.
...
Для автономного режима daemon атомарно создаёт `.visual-intent/worker-lease.json` с PID и случайным token: второй живой локальный процесс не может параллельно управлять тем же worker. Живая lease всегда блокирует конкурента; stale или повреждённая lease тоже закрывает запуск по умолчанию и удаляется только после явного `--force-recover-stale-worker`. Release удаляет только файл со своим token. `SIGINT` и `SIGTERM` передаются в SDK через `AbortSignal`: оборванный `visual-intent-owned` пакет становится `failed` с причиной `worker_interrupted`, сохраняет видимый частичный diff и допускает только явный Retry; автоматического отката файлов нет.

Отдельная `.visual-intent/daemon-lease.json` защищает весь proxy-процесс на
канонический репозиторий, независимо от режима executor. Она не даёт foreground
CLI и LaunchAgent одновременно менять общую сессию и очередь. Для переходного
периода живой daemon старой версии дополнительно обнаруживается по сверяемому
health-ответу из `connection.json`.
...
Универсальный stdio MCP и daemon могут быть разными процессами, работающими с
тем же `tasks.json`. Поэтому daemon ограниченно сверяет только автономные
`visual-intent-owned` пакеты в `queued`: если другого `in_progress` writer нет,
он идемпотентно ставит их внутреннему dispatcher. Host-attached пакеты polling
не доставляет повторно. Благодаря этому direct MCP `finish`, `retry` или
dispatch не требуют перезапуска proxy и не создают параллельного writer.
...
Точный token usage доступен напрямую для `visual-intent-owned` SDK worker и относится ко всему завершённому SDK turn одного Apply-пакета. Worker явно закрепляет модель и reasoning-политику запуска, чтобы стоимость не вычислялась по изменяемой пользовательской конфигурации задним числом. Подключённая `host-attached` задача Codex живёт за границей daemon: пока доверенный host receipt не передан, execution сохраняет `usage.availability = unavailable`. У многозадачного Apply токены принадлежат пакету целиком; overlay вычисляет только явно помеченную знаком `≈` аналитическую долю `heuristic-v1`, не превращая её в provider receipt или фактическое измерение отдельной задачи.

API-эквивалентная стоимость вычисляется в provider-specific adapter CLI сразу после получения SDK usage и сохраняется внутри того же `BatchUsage`. Receipt содержит модель, точную арифметику по token usage и неизменяемый снимок официального тарифа; overlay только отображает сохранённое значение и никогда не пересчитывает старую историю по сегодняшней цене. После гарантированного периода промотарифа adapter возвращает стоимость `unavailable`, пока снимок не обновлён из официального источника. Reasoning входит в Output, cached input входит в Input. Из-за агрегированного usage SDK расчёт использует short-context ставку для всего turn, визуально помечается знаком `≈` и честно считается оценкой, а не биллинговым документом OpenAI. Одиночный пакет проецирует точную статистику turn в одну Ready-карточку. В многозадачном пакете точный итог остаётся в заголовке, а карточки показывают интерполяцию по AI-классификации, масштабу и логарифмически приглушённому числу затронутых файлов; суммы долей сходятся с пакетными token buckets и стоимостью.
