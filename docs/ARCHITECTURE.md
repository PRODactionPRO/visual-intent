...
6. Apply атомарно создаёт один надёжный пакет, снимает Git-baseline и убирает готовые элементы из редактируемой очереди без удаления обратной связи.
7. Непустой baseline переводит пакет в `needs_input`. Overlay показывает старые dirty-файлы и принимает отдельное подтверждение только для точного fingerprint; изменившийся baseline требует повторной проверки.
8. Для `host-attached` и отключённой сессии подтверждённый пакет остаётся в `waiting_for_executor`. Подключённая задача Codex забирает его атомарно через MCP; daemon не возобновляет desktop-owned thread через SDK.
9. Только режим `visual-intent-owned` переводит пакет в `queued` и запускает отдельный SDK worker. Созданный worker можно возобновлять, потому что им владеет сам Visual Intent.
10. При claim baseline снимается повторно, а при finish сравнение отделяет `batchChangedFiles` от `preExistingDirtyFiles`.
11. Изменения рассылаются открытым overlay через аутентифицированный локальный WebSocket.
12. File adapter сериализует конкурирующие записи через lock и атомарно заменяет JSON-файл.
13. MCP-клиенты могут просматривать, подтверждать, повторять, забирать и завершать те же пакеты через daemon или совместимый файловый мост.
...
Daemon присваивает каждой создаваемой задаче репозиторий из `--repo`; payload браузера не может выбирать filesystem target. Подключение задачи Codex должно повторно передать канонический корень репозитория и отклоняется, если он отличается от сохранённой сессии. Codex dispatcher запускается с правом записи в workspace, без сетевого доступа и интерактивных подтверждений; его prompt запрещает commits, pushes, deploy, изменение credentials, создание worktrees и разрушительные действия. Незакоммиченные изменения останавливают пакет до подтверждения точного baseline в overlay. CLI-флаг `--allow-dirty` оставлен как осознанное глобальное исключение для локальной автоматизации и фиксируется в пакете как источник одобрения `cli`.
