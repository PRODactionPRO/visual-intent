...
- **VI-003 — resolved:** dirty-worktree переводит пакет в `needs_input`, overlay показывает существующие файлы и требует отдельного подтверждения точного fingerprint. Проверено в реальной браузерной proxy-сессии: `needs_input` → подтверждение → `waiting_for_executor`.
